...
int main(int argc, char **argv)
{
  ...
  int resn0 = 4, resn1 = 2, resn2 = 1; // in order of priority
  ...
  // SET SOLVER
  SOLVER ekg_solver = solve_Hptol_slow; //solve_E_fast;
...
// **********************************************************
    ...
    MAPID rfn { {WRITEDR,wr_dr}, {HYPTOL,tol}, {ELLTOL,ell_tol},
	        {EUP_WEIGHT,ell_up_weight}, {DSPN_WEIGHT,dspn} };
    set_rmap(rfn, lastpt, dr, dt, lam, rmin, rmax);

    cout << "\nr_map testing:\n" << endl;
    for (auto const& elem : r) {
      cout << elem.first << "   :   " << elem.second - rfn[elem.first] << endl;
    }
...
// **********************************************************
    ...
    if (!static_metric) {
      itn = solve_t0_slow(f_xi, f_pi, f_al, f_be, f_ps, r, lastpt, maxit);
    }
...
// *****************************************************************
	...
	if (outgoing_null_b(f_al, f_be, f_ps, r, lastpt) <= 0) {
	  cout << "\nstep " << i << "\t t = " << t << endl;
	  cout << "\nAPPARENT HORIZON FOUND at r = " << rmax << "\nk = " << lastpt;
	  cout << "\nr_areal = " << rmax*sq(f_ps[lastpt]) << "\n\ncontinue?" << endl;
	  cin >> outfile;
	}
	int k = lastpt;
	while (--k > 0) {
	...
	if (outgoing_null_f(f_al, f_be, f_ps, r, 0) <= 0) {
	  cout << "\nstep " << i << "\t t = " << t << endl;
	  cout << "\nAPPARENT HORIZON FOUND at r = " << rmin << "\nk = " << 0;
	  cout << "\nr_areal = " << rmin*sq(f_ps[0]) << "\n\ncontinue?" << endl;
	  cin >> outfile;
	}
      }
    }
