...
// **********************************************************
    ...
    lapack_int N_ab = 2*npts;
    lapack_int ldb_ab = N_ab;
    vector<lapack_int> ipiv_ab(N_ab);
    VD jac_ab(ldab*N_ab, 0), res_ab(ldb_ab, 0);
    VD jac_ab_zero(ldab*N, 0);
...
// ******************************************************************
	...
	while (res > ell_tol) {
	  ...
	  info = LAPACKE_dgbsv(LAPACK_COL_MAJOR, N_ab, kl, ku, nrhs,
			       &jac_ab[0], ldab, &ipiv_ab[0], &res_ab[0], ldb_ab);
	  if (info != 0) { cout << t << "\nERROR: cannot solve elliptic equations\ninfo = " << info << endl; }
	  apply_up_ab(res_ab, f_al, f_be, npts);
	  get_ell_res_ab_slow(res_ab, f_xi, f_pi, f_al, f_be, f_ps, r, lastpt);
	  res = norm_inf(res_ab);
	  if (++ell_itn > maxit) {
	...
	set2_cn(old_be, old_al, f_be, f_al, cn_be, cn_al, npts);
        res = get_hyp_res_ppx(res_hyp, old_xi, old_pi, old_ps, f_xi, f_pi, f_ps,
			      cn_xi, cn_pi, cn_al, cn_be, cn_ps, r, lastpt);
	if (++itn > maxit) {
