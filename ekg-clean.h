		   ...
		   const VD& cn_al, const VD& cn_be, const VD& cn_ps, MAPID& r, int lastwr, int save_pt)
{
  ...
  for (int k = 0; k < lastwr; ++k) {
    res_ps[k] = fda_hyp_resPs(old_ps, f_ps, cn_xi, cn_pi, cn_al, cn_be, cn_ps, r, s);
    s += save_pt;
  }
  res_ps[lastwr] = fdaR_hyp_resPs(f_ps, r, s);
  return;
}
		   ...
		   MAPID& r, int lastwr, int save_pt)
{
  ...
  int s = save_pt;
  for (int k = 0; k < lastwr; ++k) {
    res_ps[k] = fda_resPs(f_xi, f_pi, f_al, f_be, f_ps, r, s);
    s += save_pt;
  }
  res_ps[lastwr] = fdaR_resPs(f_ps, r, s);
  return;
}
